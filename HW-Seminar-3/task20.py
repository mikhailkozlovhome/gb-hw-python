...
my_dictionary = {'AEIOULNSTR' : 1, 'DG' : 2, 'BCMP' : 3, 'FHVWY' : 4, 'K' : 5, 'JX' : 8, 'QZ' : 10,
                 'АВЕИНОРСТ' : 1, 'ДКЛМПУ' : 2, 'БГЁЬЯ' : 3, 'ЙЫ' : 4, 'ЖЗХЦЧ' : 5, 'ШЭЮ' : 8, 'ФЩЪ' : 10}
sum_of_points = 0

for char in input("Введите слово для подсчета очко: "):
    for key in my_dictionary.keys():
        if char.upper() in key:
            sum_of_points += my_dictionary.get(key)

print(f"Сумма очков равна {sum_of_points}")